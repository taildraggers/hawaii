"""Aggregates Hawaii-located listings from every taildraggers.com
...
ones whose location ends in ", HI".
...
California/Colorado/Connecticut/Delaware/Florida/Georgia repos' approach,
with the state filter swapped to "HI".
"""
...
PAGE_TITLE = "Taildragger Ads in Hawaii"
STATE_ABBR = "HI"
...
_HAWAII_LOCATION_RE = re.compile(r",\s*HI\s*$", re.IGNORECASE)


def _is_hawaii(location: str) -> bool:
    return bool(_HAWAII_LOCATION_RE.search(location.strip()))
...
def collect_listings() -> list[Listing]:
    ...
    for repo in SOURCE_SITES:
        ...
        hawaii_rows = [row for row in rows if _is_hawaii(row.location)]
        print(f"  [{repo}] {len(rows)} listings, {len(hawaii_rows)} in Hawaii")
        all_listings.extend(hawaii_rows)
...
def render_html(listings: list[Listing]) -> str:
    ...
    rows_html = "".join(rows) if rows else (
        ...
        "No Hawaii listings found in the latest run.</td></tr>"
    )
...
def main() -> None:
    ...
    print(f"[main] total unique Hawaii listings: {len(listings)}")

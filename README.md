...
Daily aggregator of Hawaii-located taildragger classified listings,
...
[Georgia](https://github.com/taildraggers/georgia) repos' approach, with
the state filter swapped to "HI".
...
- Each fetched page's listing table is parsed back into `(title, url,
  ...
  Location cell ends in `, HI` (case-insensitive), then de-duplicated by
...
- The result is rendered into `docs/index.html` titled **"Taildragger Ads
  in Hawaii"**, using the exact same table/card layout, dark-mode
...
4. GitHub will publish the page at `https://taildraggers.github.io/hawaii/`
...
```html
<iframe
  src="https://taildraggers.github.io/hawaii/"
  title="Taildragger Ads in Hawaii"
...
- This repo could be copied/adapted for other states by changing
  `STATE_ABBR` and `_HAWAII_LOCATION_RE` in `main.py`, and updating
  `PAGE_TITLE`.
